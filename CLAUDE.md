...
3. **実装スコープの境界線(確定事項、勝手に拡張しない):**
   ...
   - 対応しない(ユーザーとの合意済み): 色弱対応の配色切替、レポート出力・共有結果のエクスポート機能。レポート出力ボタンはUIから完全に削除済み(非活性表示ではなく完全撤去)。
...
1. ~~未pushのローカル変更をコミット・push する~~ → 完了(2026-08-09、コミット`eec4d58`)。
2. ~~ピン座標が仮値~~ → 完了(2026-08-09)。ユーザーから実測値の提供を受け`script.js`の`pinDefs`を更新済み:
   桜島港フェリーターミナル (31.5964541, 130.5628203)、黒神埋没鳥居 (31.5845576, 130.7062570)。
...
6. **レポート出力ボタンを完全削除**(2026-08-09)。`index.html`から`<button id="reportBtn">`を撤去、`style.css`の未使用クラス`.btn-disabled`も削除済み。共有ボタンのみが`action-row`に残る。
7. ローカルでのブラウザ視覚確認は要実施。Live Server等で見た目を確認しながら進めること。
